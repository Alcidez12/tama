...
class SettingsRouter extends BaseRouter {
    constructor() {
        ...
        this.get('/:type/:id', async (req) => {
            try {
                let setting = await settingController.getSetting(req.params.type, req.params.id, req.account.id);
                if (!setting) {
        ...
        this.delete('/:type/:id', async (req) => {
            try {
                let setting = await settingController.deleteSetting(req.params.type, req.params.id, req.account.id);
                if (!setting) {
                    return {status: HTTPCodes.NOT_FOUND};
                }
        ...
        this.get('/:type/:id/:subtype', async (req) => {
            try {
                let subsettings = await settingController.getSubsettings(req.params.type, req.params.id, req.account.id, req.params.subtype);
        ...
        this.get('/:type/:id/:subtype/:subid', async (req) => {
            try {
                let subsetting = await settingController.getSubsetting(req.params.type, req.params.id, req.account.id, req.params.subtype, req.params.subid);
                if (!subsetting) {
                    return {status: HTTPCodes.NOT_FOUND};
                }
        ...
        this.delete('/:type/:id/:subtype/:subid', async (req) => {
            try {
                let subsetting = await settingController.deleteSubsetting(req.params.type, req.params.id, req.account.id, req.params.subtype, req.params.subid);
                if (!subsetting) {
                    return {status: HTTPCodes.NOT_FOUND};
                }
